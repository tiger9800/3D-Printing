...
def parse_export(lines):
    """
    Parse lines from an XML file generated by the Visual Machines Software.

    Paramaters
    ----------
    lines : list
        List of strings that correspond to lines of the XML file.
...
def XML_to_REDCap(params):
    """
    Function that converts the paramters from the XML into REDCap naming format. Also,
    only keep the paramters from the XML that are present in the REDCap database.
    
    Paramaters
    ----------
    params : dict 
        Dictonary of XML paramaters. 
    """
    params_new = {}
...
def params_clean_convert(params_new):
    ...
    ----------
        params_new : dict
            Dictonary returned by XML_to_REDCap function.
